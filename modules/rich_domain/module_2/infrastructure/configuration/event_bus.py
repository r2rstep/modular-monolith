    ...
    def _subscribe_notifications(self) -> None:
        for notification_cls, handler_cls in [
            (
                ...
                build_store_command_in_inbox_handler(DoSomething, self._module_1.RichDomainModelCreated),
            ),
        ]:
            self._event_bus.subscribe(notification_cls, self._container.get(handler_cls))

            self._process_outbox_handler.add_handler(
                MessageTopic(str(type(notification_cls))),
                NotificationEventMessageHandler(typing.get_args(notification_cls)[0], self._event_bus),
            )
